...
const App = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Check if it's a subtitle-only download and no subtitles were found
      if (options.downloadType === 'subtitles' && response.data.message.includes("No subtitles found")) {
        setMessage("No subtitles found for the requested language. The video might not have subtitles, or they might not be available in the selected language.");
      } else {
